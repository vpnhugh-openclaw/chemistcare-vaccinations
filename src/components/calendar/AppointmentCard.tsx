  ...
  return (
    <button
      ...
      className={cn(
        ...
        'hover:shadow-sm hover:translate-x-px active:scale-[0.99]',
        'min-h-[3.5rem]', // touch-friendly minimum height
    ...
    >
      ...
      <p className="text-sm font-semibold text-foreground mt-0.5 leading-tight truncate">{appointment.patientName}</p>
      <p className="text-xs text-muted-foreground leading-tight truncate">{appointment.service}</p>
      {appointment.status === 'completed' && appointment.notes && (
        <p className="text-[0.625rem] text-muted-foreground mt-1 line-clamp-1 italic">{appointment.notes}</p>
      )}
    </button>
  );
}
